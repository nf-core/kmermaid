...
1. [write_barcode_meta_csv](#--write_barcode_meta_csv): This creates a CSV containing the number of reads and number of UMIs per barcode, written in a path relative to `${params.outdir}/barcode_metadata`. This csv file is empty with just header when the count_valid_reads is zero i.e reads and UMIs per barcode are calculated only when the barcodes are filtered based on [count_valid_reads](#--count_valid_reads)
